...
export default function CardOrdersPage() {
  ...
  useEffect(() => {
    // Subscribe to card orders collection
    const q = query(collection(db, 'card-orders'), orderBy('createdAt', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const ordersData: CardOrder[] = [];
      snapshot.forEach((doc) => {
        ordersData.push({ id: doc.id, ...doc.data() } as CardOrder);
      });
  ...
  const handleStatusChange = async (orderId: string, newStatus: 'COMPLETED' | 'CANCELLED') => {
    try {
      const orderRef = doc(db, 'card-orders', orderId);
  ...
  const handleDelete = async (orderId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'card-orders', orderId));
    } catch (error) {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    return matchesSearch && matchesStatus && matchesChallengeStatus;
  });
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <CreditCard className="text-[#0FF1CE]" />
            Card Orders Management
          </h1>
          <p className="text-gray-400 text-sm mt-1">Monitor and manage all credit/debit card orders</p>
        </div>
        <button
                  ...
                  <tr key={order.id} className="border-b border-[#2F2F2F]/50 hover:bg-[#151515]">
                    ...
                    <td className="px-4 py-4">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full font-medium flex items-center gap-1">
                          <CreditCard size={10} />
                          CARD
                        </span>
                      </div>
